  ...
  useEffect(() => {
    ...
    if (c) {
      fetch(`http://localhost:3000/api/sessoes/by-code/${c}`)
        .then(r => r.json())
        .then(data => {
          if (data?.aventuraSnapshot?.titulo) setTituloSessao(data.aventuraSnapshot.titulo);
        })
        .catch(() => {});
    }
  }, []);
      ...
      <div className="btn-entrar-aventura-container">
        <button
          ...
          onClick={async () => {
            ...
            try {
              const res = await fetch(`http://localhost:3000/api/sessoes/by-code/${codigo}/alunos`, {
                method: 'POST',
              ...
              navigate(`/escolher-classe?codigo=${codigo}&nome=${encodeURIComponent(nome)}`);
            } catch (err) {
